...
PYBIND11_MODULE(lazy_tree_original, m) {
    m.doc() = "Original Lazy Search Tree Bindings";
    ...
    m.def("run_custom", [](const std::string& tree_type, const std::vector<int>& preload, const std::vector<int>& inserts, const std::vector<int>& queries) {
        ...
        if (tree_type == "BTree") {
            ...
            return run_custom_benchmark_cpp<splay_tree<int, CountingComp<int>>>(preload, inserts, queries, max_key_hint);
        } else {
    ...
    m.def("run_custom_interleaved", [](const std::string& tree_type, const std::vector<int>& preload, const std::vector<int>& inserts, const std::vector<int>& queries) {
        ...
        if (tree_type == "BTree") {
            ...
            return run_custom_interleaved_cpp<splay_tree<int, CountingComp<int>>>(preload, inserts, queries, max_key_hint);
        } else {
    ...
    m.def("run_custom_interleaved_delete", [](const std::string& tree_type, const std::vector<int>& preload, const std::vector<int>& inserts, const std::vector<int>& queries) {
        ...
        if (tree_type == "BTree") {
            ...
            return run_custom_interleaved_delete_cpp<splay_tree<int, CountingComp<int>>>(preload, inserts, queries, max_key_hint);
        } else {
